...
```
 01_input/  ──▶  02_function/  ──▶  03_output/
 (공유 원자료)     (JD·포지셔닝·도구)    (이력서·자소서·포트폴리오)
```

**하나의 input, 회사마다 다른 output.** 내 경력·프로젝트·Jay 멘토링 지식이라는 같은 원자료를,
각 회사의 JD 라는 함수에 통과시켜 조준된 지원서를 만든다.

핵심 정체성: *"AI를 실제 필드에 배포(deploy)하고, 남들이 그걸 할 수 있게 가르치는 사람"*

> 창업 트랙(Physical Revolt)은 `~/workspace/physical-playground` 독립 프로젝트로 이관됐다.
> 관련 문서는 `99_archive/` 에 보관. 이 repo 는 **지원서 생성기**로 역할을 좁혔다.
...
| 바뀌는 것 | 처음 (잘못) | 지금 |
|---|---|---|
| 이력서 문장 | 파이썬 스크립트에 하드코딩 | **md 파일** (`03_output/…/이력서_….md`) |
| 워드 서식 | 스크립트가 스타일을 재현 | **템플릿 docx** (`02_function/templates/`) |
...
| 경로 | 역할 |
|---|---|
| **`01_input/`** | **공유 원자료.** 경력 기록, 포트폴리오, 면접 전사록, 멘토링 지식. |
| `01_input/제출이력.md` | ★ **output → input 되먹임.** 어디에 무엇을 어떤 서사로 냈는지. **새 이력서를 쓰기 전 이 파일부터 읽는다.** |
| `01_input/Jay/` | Jay Kim 지식 베이스. `INDEX.md`(원칙 요약 — AI에게 먼저 줄 파일) → `knowledge/`(K1~K7 카드) → `by-topic/` → `raw/`(전사록). |
| `01_input/Interview_note/` | 실제 면접·스크리닝 콜 전사록. |
| **`02_function/`** | **변환 함수.** JD 원문(`*_JD_*.md`), 포지셔닝 전략, 이력서/자소서/포트폴리오 작성 맥락, 자산 인벤토리. |
| **`02_function/templates/`** | **이력서·자소서 표준 서식 docx.** 워드에서 다듬은 서식을 자산화한 것. |
| **`02_function/tools/`** | md → docx 변환기 등 기계적 도구. |
| **`03_output/submitted/`** | **제출 완료.** `YYYY-MM-DD_회사_직무/` + 각 폴더에 `SUBMITTED.md`. `INDEX.md` 로 시간순 조회. |
| **`03_output/drafts/`** | **작성 중.** 제출하면 `submitted/` 로 옮긴다. |
| `03_output/portfolio-bithabit/` | 포트폴리오 사이트 소스 (**별도 git repo** — push 하면 bit-habit.com 에 배포). |
| `04_viewer/` | 로컬 문서 뷰어. `뷰어시작.command` 더블클릭 → http://localhost:8765 |
| `99_archive/` | 이관·보류된 트랙 (창업 Physical Revolt 등). |
| `99_backup_20260615/` | 초기 스냅샷 (전략 전환 이전). 변화 추적용. |

최상위 폴더의 번호는 **파이프라인 순서**다: `01_input` → `02_function` → `03_output`.
도구(`04_viewer`)와 아카이브(`99_`)가 뒤를 잇는다.

### JD 는 왜 `02_function/` 에 있나
...
### 1. JD 를 원문 그대로 보존
`02_function/<회사>_JD_<식별자>.md` — 인용 블록으로 원문 전체 + 요구역량 정리 + `나중에 볼 포인트`.
`상태: 📥 보관만` 같은 줄을 넣어 두면 `grep "보관만"` 으로 미착수 JD 를 찾을 수 있다.

### 2. 기존 자산에서 출발
`01_input/제출이력.md` 와 최근 제출본을 먼저 읽는다. 경력 본문은 사실이라 **재사용 가능**하지만,
...
`03_output/submitted/YYYY-MM_회사_직무/` 로 옮기고 `SUBMITTED.md` 를 쓴다.
...
### `02_function/tools/md2docx_template.py` — 이력서 (주력)

```bash
python3 02_function/tools/md2docx_template.py <입력.md> [출력.docx] [--template <서식.docx>]
```

`02_function/templates/이력서_표준서식.docx` 를 열어 **본문만 갈아끼운다.**
...
- **불릿** = 글머리표가 붙은 단락
- **본문(산문)** = 제목 이후의 글머리표 없는 문단 (자기소개서용)
- **헤더** = 첫 제목 이전의 단락들

→ 템플릿을 갈아도 코드는 그대로. 새 서식이 생기면 `02_function/templates/` 의 docx 만 교체한다.

**파일명에 "자기소개서"가 있으면 자소서 템플릿을 자동으로 고른다.** `--template` 은 예외적일 때만.

### 표준 서식

| 템플릿 | 출처 |
|---|---|
| `이력서_표준서식.docx` | 2026-07-13 LG U+ 지원본 (워드에서 다듬은 판) |
| `자기소개서_표준서식.docx` | 2026-07-13 Upstage 218421 제출본 |
...
| 프로젝트 | `#### BookToss — url` | **Heading 3** |
| 불릿 | `- 항목` | **List Bullet 9pt**, `**리드인**` 굵게 |

자소서는 여백이 더 넉넉하고(1008 vs 680) 본문이 산문이다 — **이력서는 스캔하는 문서, 자소서는 읽는 문서.**

> **워드에서 손본 것을 잃지 않으려면:** **구조**를 바꿨으면(예: 프로젝트 제목을 Heading 3 으로) md 에도
> 반영해야 다음 빌드에서 안 날아간다. **서식만** 바꿨으면 그 docx 를 템플릿으로 승격하면 된다.
...
소스: `03_output/portfolio-bithabit/` (별도 repo, `git push` → GitHub Actions → 서버 pull)
...
- **폴더는 영어**(`drafts` / `submitted`), 제출 폴더는 `YYYY-MM-DD_회사_직무`, **파일명 끝에 `_YYYYMMDD_HHMM`**(생성 시각)
...
| 프로젝트 | 설명 | Repo |
|---|---|---|
| portfolio-bithabit | 포트폴리오 허브 (`03_output/portfolio-bithabit/`) | [bookseal/portfolio-bithabit](https://github.com/bookseal/portfolio-bithabit) |
