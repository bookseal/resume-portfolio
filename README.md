...
```
                    ┌──▶ 02_function/job/     ──▶ 03_output/job/     (Upstage, Cognition, LG U+)
 01_input/  (공유 원자료) ─┤
                    └──▶ 02_function/startup/ ──▶ 03_output/startup/ (피칭·트랙션)
```
...
| 바뀌는 것 | 처음 (잘못) | 지금 |
|---|---|---|
| 이력서 문장 | 파이썬 스크립트에 하드코딩 | **md 파일** (`03_output/job/…/이력서_….md`) |
...
| **`02_function/job/`** | **취업 변환 함수.** JD 원문, 포지셔닝 전략, 이력서/자소서/포트폴리오 작성 맥락, 자산 인벤토리. |
| **`02_function/templates/`** | **이력서 표준 서식 docx.** 워드에서 다듬은 서식을 자산화한 것. |
| **`02_function/tools/`** | md → docx 변환기 등 기계적 도구. |
| `02_function/startup/` | 창업 변환 함수 (현재 `~/workspace/physical-playground` 로 이관). |
| **`03_output/job/submitted/`** | **제출 완료.** `YYYY-MM_회사_직무/` + 각 폴더에 `SUBMITTED.md`. `INDEX.md` 로 시간순 조회. |
| **`03_output/job/drafts/`** | **작성 중.** 제출하면 `submitted/` 로 옮긴다. |
...
### 1. JD 를 원문 그대로 보존
`02_function/job/<회사>_JD_<식별자>.md` — 인용 블록으로 원문 전체 + 요구역량 정리 + `나중에 볼 포인트`.
...
`03_output/job/submitted/YYYY-MM_회사_직무/` 로 옮기고 `SUBMITTED.md` 를 쓴다.
